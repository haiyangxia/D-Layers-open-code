...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, x):
        flat_x = x.repeat_interleave(self.embedding_dim, dim=1)
        flat_x = flat_x.reshape(-1, self.embedding_dim)
...
out_path = Path('./adv_samples/VQ-VAE/dup')
for dataset_name in dataset_list:
    ...
    for atk_name, atk_raw in adv_samples.items():
        ...
        with open(out_path/f'../vae_dup_defense.csv', mode='a') as f:
            f.write(','.join([
                dataset_name,
                atk_name,
                'Qlayer-VQ-VAE-DUP',
