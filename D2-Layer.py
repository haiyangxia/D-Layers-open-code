...
class D2Net(nn.Module):
    ...
    def descritization(self, x, levels=5):
        ...
        labels_numpy = levels.cpu().detach().numpy()
        X_dis = transformer.fit_transform(x_numpy, labels_numpy)
        return torch.tensor(X_dis, dtype=torch.float32, requires_grad=True)

    def forward(self, x, labels):
        x = torch.nan_to_num(x, nan=0)
        if self.training:
            labels = torch.max(labels, 1)[1]
        h1 = self.relu(self.linear1(self.descritization(x, labels)))
        h2 = self.relu(self.linear2(h1))
        h3 = self.relu(self.linear2(h2))
        h4 = self.relu(self.linear2(h3))
        h5 = self.relu(self.linear2(h4))
        ...
        if es:
            if min_loss > current_loss:
                ...
                if cur_es_count >= es_count:
                    ...
                    # plt.plot(list(range(len(smoothed))), smoothed, color='#0ECA7D', label='acc train')
                    # plt.show()
                    return model, epoch
        if (epoch % 50 == 0):
            print("> epoch {:.0f}\tLoss {:.5f}\tAcc {:.5f}".format(epoch, current_loss, current_correct))
    # np.save("./d2-coverge-performance/" + dataset_name + "_" + modelName + ".npy")
    return model, epochs
...
def torch_predict(model, data, y_test_sample):
    model.eval()
    y_pred = model(data_tensor,  torch.FloatTensor(y_test_sample)).max(1)[1]
    return y_pred

def evaluate_data(model, y_true, data, pure_pred=None, log=True):
    y_pred = model(data_tensor,  torch.FloatTensor(y_true)).max(1)[1]
...
for dataset_name in dataset_list:
    ...
    d2layer_model, _ = train_model(X_train, y_train, device, 'd2layer')
    d2layer_model.eval()
    y_pred = d2layer_model(torch.FloatTensor(X_test), torch.FloatTensor(y_test))
    ...
    y_test_sample_pred = torch_predict(d2layer_model, X_test_sample, y_test_sample)
    ...
    for atk_name, atk_raw in adv_samples.items():
        ...
        with open(out_path/f'./D2_MDL_defense.csv', mode='a') as f:
            f.write(','.join([
                dataset_name,
                atk_name,
                'D2',
